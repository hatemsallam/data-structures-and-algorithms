...
const longestString = (arr) => {
  const oldArray = [...arr];
  const longestString = oldArray.sort((a, b) => b.length - a.length);
...
const findHappiness = (arr) => {
  let modifiedArray = arr.filter((item) => item.includes(":)"));
  return modifiedArray;
};
...
const standardizePhoneNumbers = (arr) => {
  let modifiedArray = arr.map((index) => {
    let X = index.split("");
    X.splice(0, 1);
    X.splice(3, 1);
    X.splice(3, 1);
...
const onlyOddChars = (str) => {
  let modifiedArray = str.split("").map((odd, index) => {
    if (index % 2 != 0) {
      return odd;
    }
  });
