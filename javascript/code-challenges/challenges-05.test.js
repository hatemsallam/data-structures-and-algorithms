...
const toLastNames = people => {
  ...
  people.map((item, index) => {
    newListPeople[index] = `${item.firstName} ${item.lastName}`;
  });
...
const addValues = (arr) => {
  let summation = arr.reduce((accumulator, value) => {
...
const countNumberOfElements = (arr) => {
  let number = arr.reduce((accumulator, value, index) => {
    accumulator = index + 1;
    return accumulator;
  }, 0);
  return number;
...
const returnNames = (arr) => {
  let namesArray = arr.reduce((accumulator, item, index) => {
