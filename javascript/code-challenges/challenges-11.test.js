...
function transformToLis(obj){
  let modifiedArray = [];
  for (let key in obj) {
    modifiedArray.push(`<li>${key}: ${obj[key]}</li>`);
  }
...
const totalSum = (input) => {
  const mapArr = input.map(arr => arr.reduce((acc, cur) => acc += cur, 0));
  return mapArr.reduce((acc, cur) => acc += cur, 0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const powerArr = divisibleByFive.map(arr => arr.reduce((acc, cur) => {
...
let findMaleAndFemale = (data) => {
  const gender = data.filter(obj => (obj.gender === 'male' || obj.gender === 'female'));
  const names = gender.reduce((acc, cur) => {
    acc.push(cur.name);
    return acc;
  }, [])
